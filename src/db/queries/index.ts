...
import { and, between, sql, desc, inArray } from "drizzle-orm";
...
export async function getTopTrendingItems(
  itemCount: number = 10, 
  ...
  const convertedResult: TrendingItem[] = result.map(item => ({
    ...
    eventCount: Number(item.totalCount), // Maintain backward compatibility
  }));
